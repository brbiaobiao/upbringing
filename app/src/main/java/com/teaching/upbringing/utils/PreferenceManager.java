...
//        Log.d("remove","删除");
    ...
    private static SharedPreferences openSharePreference() {
        return  AppApplication.getApp().getSharedPreferences(PREFERENCE_PRIVATE_SP, Context.MODE_PRIVATE);
    }
    ...
    public static boolean saveObject(Serializable obj, @NonNull String fileName) {
        ...
        try {
            if (obj != null) {
                File pathFile = AppApplication.getApp().getFilesDir();
                if (pathFile != null) {
                    pathFile.mkdirs();
    ...
    public static final class Editor_ {
        ...
        private Editor_(@NonNull String preferenceName) {
            sp = AppApplication.getApp().getSharedPreferences(preferenceName, Context.MODE_PRIVATE);
            ed = sp.edit();
        }
