 ...
 */
package com.outsourcing.library.crash.log;
...
public class CrashCatcher implements Thread.UncaughtExceptionHandler {
     ...
     * @param handler
     */
    public void init(Context context, String logFileDir, CrashListener listener, Handler handler, Thread.UncaughtExceptionHandler mDefaultHandler) {
        mContext = context;
...
//        long timestamp = System.currentTimeMillis();
    ...
    private boolean handleException(final Thread thread, final Throwable ex, final CrashCatcher crashCatcher) {
        ...
        try {
            ...
            LogWriter.writeLog(crashCatcher.mContext, crashCatcher.mLogFile, ex);
        } catch (Exception e) {
            return false;
        }
    ...
    public static void writeLog(final Throwable ex) {
        ...
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                CrashCatcher.getInstance().mLogFile = CrashCatcher.getInstance().createLogFile();
                LogWriter.writeLog(CrashCatcher.getInstance().mContext, CrashCatcher.getInstance().mLogFile, ex);
            }
        });
    }
}
